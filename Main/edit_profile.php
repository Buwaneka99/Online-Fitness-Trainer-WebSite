...
else
{
    ...
    if(isset($_POST['update']))
    {
        ...
        $sql = "UPDATE user SET firstname = ?, lastname = ?, email = ?, address = ?, contactno = ? , username = ? WHERE username = ?";

        $stmt = $con->prepare($sql);
        $stmt->bind_param("sssssss", $firstname, $lastname, $email, $address, $contactno , $username , $username1);
